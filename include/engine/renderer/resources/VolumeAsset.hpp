...
namespace RtEngine {
    ...
    struct VolumeAsset {
        ...
        static std::shared_ptr<VolumeAsset> createHomogenous(const std::string &name, const float absorption, const float scattering, const float g, const std::shared_ptr<MeshAsset> &bounding_mesh) {
            ...
            result->volume_buffers = std::make_shared<VolumeBuffers>(
                glm::uvec3(10, 10, 10),
                std::vector<glm::vec2>(1000, glm::vec2(absorption, scattering))
            );
