...
struct PropertiesSection {
    ...
    void addInt(const std::string& name, int32_t* var, uint32_t flags = ALL_PROPERTY_FLAGS, int32_t min = 0, int32_t max = -1)
    {
        auto property = std::make_shared<IntProperty>(name, var, flags, min, max);
        int_properties.push_back(property);
    }

    void addFloat(const std::string& name, float* var, uint32_t flags = ALL_PROPERTY_FLAGS, float min = 0, float max = -1)
    {
    ...
    void addSelection(const std::string& name, std::string* var, std::vector<std::string> selection_options, uint32_t flags = ALL_PROPERTY_FLAGS)
    {
