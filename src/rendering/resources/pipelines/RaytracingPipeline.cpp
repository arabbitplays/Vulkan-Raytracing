#include "../../../../include/rendering/resources/pipelines/RaytracingPipeline.hpp"
...
namespace RtEngine {
    ...
    void RaytracingPipeline::build() {
        ...
        if (CreateRayTracingPipelinesKHR(context->device_manager->getDevice(), VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &handle) !=
            VK_SUCCESS) {
	...
	void RaytracingPipeline::addShaderStage(VkShaderModule shaderModule, VkShaderStageFlagBits shaderStage,
    	...
    	Pipeline::addShaderStage(shaderModule, shaderStage);
