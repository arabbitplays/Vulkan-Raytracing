...
void SceneManager::createBlas(std::vector<std::shared_ptr<MeshAsset>>& meshes) {
    ...
    for (auto& meshAsset : meshes) {
        meshAsset->accelerationStructure = std::make_shared<AccelerationStructure>(device, *vulkan_context->resource_builder, *vulkan_context->command_manager, VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR);
...
void SceneManager::createSceneDescriptorSets(const VkDescriptorSetLayout& layout)
{
    for (int i = 0; i < max_frames_in_flight; i++) {
        scene_descriptor_sets.push_back(vulkan_context->descriptor_allocator->allocate(vulkan_context->device_manager->getDevice(), layout));
    }
}
...
void SceneManager::createUniformBuffers() {
    ...
    for (size_t i = 0; i < max_frames_in_flight; i++) {
        sceneUniformBuffers[i] = vulkan_context->resource_builder->createBuffer(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

        vkMapMemory(vulkan_context->device_manager->getDevice(), sceneUniformBuffers[i].bufferMemory, 0, size, 0, &sceneUniformBuffersMapped[i]);

        scene_resource_deletion_queue.pushFunction([&, i]() {
            vulkan_context->resource_builder->destroyBuffer(sceneUniformBuffers[i]);
        });
    }
}
...
void SceneManager::updateSceneDescriptorSets(uint32_t frame_idx, const AllocatedImage& current_image, const AllocatedImage& rng_tex) {
    ...
    if (bufferUpdateFlags & GEOMETRY_UPDATE)
    {
        vulkan_context->descriptor_allocator->writeAccelerationStructure(0, tlas_handle, VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR);
        vulkan_context->descriptor_allocator->writeBuffer(3, geometry_manager->getVertexBuffer().handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        vulkan_context->descriptor_allocator->writeBuffer(4, geometry_manager->getIndexBuffer().handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        vulkan_context->descriptor_allocator->writeBuffer(5, geometry_manager->getGeometryMappingBuffer().handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        vulkan_context->descriptor_allocator->writeBuffer(6, instance_manager->getInstanceBuffer().handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        vulkan_context->descriptor_allocator->writeBuffer(7, instance_manager->getEmittingInstancesBuffer().handle, 0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);

        std::vector<VkImageView> views{};
        for (uint32_t i = 0; i < 6; i++) {
            views.push_back(scene->environment_map[i].imageView);
        }
        vulkan_context->descriptor_allocator->writeImages(8, views, defaultSamplerLinear, VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    ...
    vulkan_context->descriptor_allocator->writeImage(1, current_image.imageView, VK_NULL_HANDLE, VK_IMAGE_LAYOUT_GENERAL, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
    vulkan_context->descriptor_allocator->writeBuffer(2, sceneUniformBuffers[frame_idx].handle, sizeof(SceneData), 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
    vulkan_context->descriptor_allocator->writeImage(9, rng_tex.imageView, VK_NULL_HANDLE, VK_IMAGE_LAYOUT_GENERAL, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
...
void SceneManager::createDefaultTextures() {
    ...
    whiteImage = vulkan_context->resource_builder->createImage((void*)&white, VkExtent3D{ 1, 1, 1 }, VK_FORMAT_R8G8B8A8_SRGB,
                               VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_ASPECT_COLOR_BIT);

    uint32_t grey = glm::packUnorm4x8(glm::vec4(0.66f, 0.66f, 0.66f, 1));
    greyImage = vulkan_context->resource_builder->createImage((void*)&grey, VkExtent3D{ 1, 1, 1 }, VK_FORMAT_R8G8B8A8_SRGB,
                                              VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_ASPECT_COLOR_BIT);

    uint32_t black = glm::packUnorm4x8(glm::vec4(0, 0, 0, 0));
    blackImage = vulkan_context->resource_builder->createImage((void*)&black, VkExtent3D{ 1, 1, 1 }, VK_FORMAT_R8G8B8A8_SRGB,
                                               VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_ASPECT_COLOR_BIT);
    ...
    errorCheckerboardImage = vulkan_context->resource_builder->createImage(pixels.data(), VkExtent3D{16, 16, 1}, VK_FORMAT_R8G8B8A8_SRGB,
                                           VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_ASPECT_COLOR_BIT);
