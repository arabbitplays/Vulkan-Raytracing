...
namespace RtEngine {
    ...
    AllocatedImage VolumeManager::createVolumeTexture(const std::shared_ptr<VolumeBuffers> &volume_buffers) const {
        ...
        return vulkan_context->resource_builder->createImage(volume_buffers->coefficients.data(), extent, VK_FORMAT_R32G32_SFLOAT, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT, VK_IMAGE_ASPECT_COLOR_BIT);
    }
