...
AccelerationStructureBuilder::AccelerationStructureBuilder(VkDevice device, RessourceBuilder ressource_builder, CommandManager command_manager) {
    this->device = device;
...
VkTransformMatrixKHR transform_matrix = {
    ...
    accelerationStructureInstance.accelerationStructureReference = blas[0].deviceAddress;
    ...
    accelerationStructure.buffer = ressource_builder.createBuffer(
        accelerationStructureBuildSizesInfo.accelerationStructureSize,
    ...
    VkAccelerationStructureCreateInfoKHR accelerationStructureCreateInfo{};
    accelerationStructureCreateInfo.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_CREATE_INFO_KHR;
    accelerationStructureCreateInfo.buffer = accelerationStructure.buffer.handle;
    accelerationStructureCreateInfo.size = accelerationStructureBuildSizesInfo.accelerationStructureSize;
    accelerationStructureCreateInfo.type = VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_KHR;
    if (CreateAccelerationStructureKHR(device, &accelerationStructureCreateInfo, nullptr, &accelerationStructure.handle) != VK_SUCCESS) {
        throw std::runtime_error("failed to create bl acceleration structure!");
    }
    ...
    VkAccelerationStructureDeviceAddressInfoKHR accelerationStructureDeviceAddressInfo{};
    accelerationStructureDeviceAddressInfo.sType = VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_DEVICE_ADDRESS_INFO_KHR;
    accelerationStructureDeviceAddressInfo.accelerationStructure = accelerationStructure.handle;
    accelerationStructure.deviceAddress = GetAccelerationStructureDeviceAddressKHR(device, &accelerationStructureDeviceAddressInfo);

    ressource_builder.destroyBuffer(instanceBuffer); // TODO maybe sus
