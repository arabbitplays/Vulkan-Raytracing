...
// TODO this is not bilinear but idk how to elegantly redistribute the weight of bad taps
vec3 taaBilinear(SvgfData data, vec2 hist_screen_coord, inout bool is_valid) {
    ...
    for (uint i = 0; i < 2; i++) {
        for (uint j = 0; j < 2; j++) {
            uint idx = (tab_screen_coord.y + i) * gl_LaunchSizeEXT.x + (tab_screen_coord.x + j);
            SvgfHistData hist_data = getSvgfHistData(idx);

            if (isSameSurface(data, hist_data)) {
                filtered_color += hist_data.color;
                valid_tabs++;
            }
        }
    }
    ...
    if (valid_tabs > 0) {
        is_valid = true;
        return filtered_color / valid_tabs;
    } else {
        is_valid = false;
        return vec3(0);
    }
...
void temporalFiltering(vec3 color, inout SvgfData data) {
    ...
    vec3 hist_color = taaNearestNeighbor(data, prev_screen_coord, valid_hist);
