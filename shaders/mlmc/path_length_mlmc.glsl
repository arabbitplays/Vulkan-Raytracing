...
vec3 pathLengthDiffMlmc(uint sample_count, uint biased_path_length, uint unbiased_path_length) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        while (payload.depth < biased_path_length && payload.next_dir != vec3(0.0) && length(payload.rr_beta) > 0) {
            continuePath();
        }
        //vec3 biased_color = payload.light;
        while (payload.depth < unbiased_path_length && payload.next_dir != vec3(0.0) && length(payload.rr_beta) > 0) {
            continuePath();
        }
