...
vec3 estimateTransmittance(vec3 P, vec3 L, float distance_to_light, int volume_idx, float sampled_dist, inout uvec4 rng_state) {
    shadow_payload.rng_state = rng_state;
    shadow_payload.transmittance = vec3(1);
    shadow_payload.dist_left = distance_to_light;
    shadow_payload.next_distance = sampled_dist;
...
vec3 estimateTransmittance(vec3 P, vec3 L, float distance_to_light, inout uvec4 rng_state) {
    return estimateTransmittance(P, L, distance_to_light, -1, distance_to_light, rng_state);
}
